...
void eval(char *cmdline) 
{
    ...
    if (!builtin_cmd(argv)) {
        // block SIGCHLD
        if (sigprocmask(SIG_BLOCK, &mask_child, &prev_child) < 0) {
            fprintf(stderr, "signal error: %s\n", strerror(errno));
            exit(0);
        }
        if ((pid = fork()) == 0) {
            // child
            if (sigprocmask(SIG_SETMASK, &prev_child, NULL) < 0) { // unblock child
                fprintf(stderr, "signal error: %s\n", strerror(errno));
                exit(0);
            }
            setpgid(0,0);
        ...
        if (sigprocmask(SIG_BLOCK, &mask_all, NULL) < 0) { // block all signals
            fprintf(stderr, "signal error: %s\n", strerror(errno));
            exit(0);
        }
        addjob(jobs, pid, bg+1, cmdline);
        if (sigprocmask(SIG_SETMASK, &prev_child, NULL) < 0) { // unblock
            fprintf(stderr, "signal error: %s\n", strerror(errno));
            exit(0);
        }
...
void do_bgfg(char **argv) 
{
    ...
    if (next_arg[0] == '%') {
        // job id
        int jid = atoi(((char *)next_arg) + 1); // read job id
...
void sigchld_handler(int sig) 
{
    ...
    sigset_t mask, prev;
    pid_t pid;
    if (sigfillset(&mask) < 0) { // init signal
        fprintf(stderr, "signal error: %s\n", strerror(errno));
        exit(0);
    }
    while ((pid = waitpid(-1, NULL, WNOHANG)) > 0) { // reap
        if (sigprocmask(SIG_BLOCK, &mask, &prev) < 0) { // block signal
            fprintf(stderr, "signal error: %s\n", strerror(errno));
            exit(0);
        }
        deletejob(jobs, pid);
        if (sigprocmask(SIG_SETMASK, &prev, NULL) < 0) { // unblock
            fprintf(stderr, "signal error: %s\n", strerror(errno));
            exit(0);
        }
    }
