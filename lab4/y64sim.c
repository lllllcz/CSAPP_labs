...
stat_t nexti(y64sim_t *sim)
{
    ...
    regid_t myRegA = REG_NONE;
    regid_t myRegB = REG_NONE;
    if ((icode >= I_RRMOVQ && icode <= I_MRMOVQ) || icode == I_ALU
        || icode == I_PUSHQ || icode == I_POPQ)
    {
        byte_t regNum = REG_NONE;
        if (!get_byte_val(sim->m, next_pc, &regNum)) {
            err_print("PC = 0x%lx, Invalid instruction address", next_pc);
            return STAT_ADR;
        }
        myRegA = GET_REGA(regNum);
        myRegB = GET_REGB(regNum);
        next_pc++;
    }

    /* get immediate if needed (8 bytes) */
    long_t myImm;
    if (icode == I_IRMOVQ || icode == I_JMP || icode == I_CALL
        || icode == I_MRMOVQ || icode == I_RMMOVQ)
    {
        if (!get_long_val(sim->m, next_pc, &myImm)) {
            err_print("PC = 0x%lx, Invalid instruction address", next_pc);
            return STAT_ADR;
        }
        next_pc += 8;
    ...
    long_t value, addrInReg, argA, argB;
    alu_t ifun = GET_FUN(codefun);
    cond_t icon = GET_FUN(codefun);
    ...
    switch (icode) {
      ...
      case I_RRMOVQ:  /* 2:x regA:regB */
        if (cond_doit(sim->cc, icon)) {
            value = get_reg_val(sim->r, myRegA);
            set_reg_val(sim->r, myRegB, value);
            sim->pc = next_pc;
        }
        else {
            sim->pc = next_pc;
      ...
      case I_IRMOVQ: /* 3:0 F:regB imm */
        set_reg_val(sim->r, myRegB, myImm);
        sim->pc = next_pc;
        break;

      case I_RMMOVQ: /* 4:0 regA:regB imm */
        value = get_reg_val(sim->r, myRegA);
        addrInReg = get_reg_val(sim->r, myRegB) + myImm;
        if (!set_long_val(sim->m, addrInReg, value)) {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, addrInReg);
            return STAT_ADR;
        }
        sim->pc = next_pc;
        break;
      ...
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        addrInReg = get_reg_val(sim->r, myRegB) + myImm;
        if (!get_long_val(sim->m, addrInReg, &value)) {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, addrInReg);
            return STAT_ADR;
        }
        set_reg_val(sim->r, myRegA, value);
        sim->pc = next_pc;
        break;

      case I_ALU: /* 6:x regA:regB */
        argA = get_reg_val(sim->r, myRegA);
        argB = get_reg_val(sim->r, myRegB);
        value = compute_alu(ifun, argA, argB);
        set_reg_val(sim->r, myRegB, value);
        sim->cc = compute_cc(ifun, argA, argB, value);
        sim->pc = next_pc;
        break;
      ...
      case I_CALL: /* 8:x imm */
        // push
        addrInReg = get_reg_val(sim->r, REG_RSP) - 8;
        set_reg_val(sim->r, REG_RSP, addrInReg);
        if (!set_long_val(sim->m, addrInReg, next_pc)) {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, addrInReg);
            return STAT_ADR;
        }
        // jmp
        sim->pc = myImm;
      ...
      case I_RET: /* 9:0 */
        // pop
        addrInReg = get_reg_val(sim->r, REG_RSP);
        // get long value and set
        if (!get_long_val(sim->m, addrInReg, &value)) {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, addrInReg);
            return STAT_ADR;
        }
        // rsp + 8
        set_reg_val(sim->r, REG_RSP, addrInReg+8);
        // jmp
        sim->pc = value;
        break;
